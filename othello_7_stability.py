...
def alphabeta(brd,tkn,lowerBnd,upperBnd):
    ...
    for mv in p_moves:
        ...
        lowerBnd = score+ 1
    return bestSoFar
def specialEval(brd,tkn,eTkn,stability):
...
def midgame_alpha_beta(brd,tkn,lowerBnd, upperBnd,level,stability):
    current_stability = stability
    eTkn = 'x' if tkn=='o' else 'o'
    p_moves = determineMoves(brd,tkn)[0]
    if level ==0:
        return specialEval(brd,tkn,eTkn,current_stability)
    if not p_moves:
        if not determineMoves(brd,eTkn)[0]:
            return [brd.count(tkn) -brd.count(eTkn)]
        ab = midgame_alpha_beta(brd,eTkn,-upperBnd,-lowerBnd,level-1,current_stability)
        return [-ab[0]] + ab[1:] + [-1]
    bestSoFar = [lowerBnd-1]
    corners_in_moves = []
    for corner in corners:
    ...
    for mv in p_moves:
        ...
        for flip in flips:
                if is_stable(new_brd,tkn,flip,[False]*64):
                    current_stability+=1
        ab = midgame_alpha_beta(new_brd.lower(),eTkn,-upperBnd,-lowerBnd,level-1,current_stability)
        current_stability = stability
        score = -ab[0]
        if score<lowerBnd:continue
        if score>upperBnd: return [score]
...
def main():
    ...
    # if board.lower().count('.')<=HLLIM:
        ...
        mab_res = midgame_alpha_beta(board.lower(),tokenToPlay,-65,65,3,count_stable_disks(board.lower(),tokenToPlay))#3 also works
